...
public class PagingDao {
    ...
    public List<Product> viewAllProduct(int offset, int noOfRecords, String email, String s, String type) {
        ...
        try {
            ...
            if (!"".equals(type)) {
                String query2 = "select id from type where name=?";
                ...
                if (s == null) {
                    query = "select p.name,p.id, p.price ,p.user_id ,t.name as type,p.create_at ,pic.url from product as p  inner join type t on p.type_id = t.id inner join picture pic on pic.id = p.image_id where p.user_id = ?  and  t.id = ? limit " + noOfRecords + " OFFSET " + offset;

                } else {
                    query = "select p.name,p.id, p.price ,p.user_id ,t.name as type,p.create_at ,pic.url from product as p  inner join type t on p.type_id = t.id inner join picture pic on pic.id = p.image_id where p.user_id = ? and t.id = ? and ( p.name like '%" + s + "%' or  t.name like '%" + s + "%' ) limit " + noOfRecords + " OFFSET " + offset;
                }
            ...
            if(!"".equals(type)){
                if (s == null) {
                    sql = "SELECT COUNT(*) as tong from product where user_id =? and type_id = ?";
                    ps = connection.prepareStatement(sql);
                    ps.setInt(1, id);
                    ps.setInt(2, type_id);
                } else {
                    sql = "SELECT COUNT(*) as tong from product  where user_id =?  and type_id = ? and ( product.name like '%" + s + "%' )";
                    ps = connection.prepareStatement(sql);
                    ps.setInt(1, id);
                    ps.setInt(2, type_id);
